...
```{r}
df_bookings %>% filter(ArrivalDate >= "2016-01-01" & ArrivalDate <= "2016-12-31") %>% group_by(week = week(ArrivalDate)) %>% summarise(mean_week = mean(AverageDailyRate)) %>% 
...
ggplot(df_bookings, aes(x = AverageDailyRate, y = LeadTime)) + geom_point()
```
