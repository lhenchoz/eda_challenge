...
df_bookings <- df_bookings %>% mutate(HouseholdType = case_when(Adults == 1 & Children == 0 & Babies == 0 ~ 'Single',
                                                           Adults == 2 & Children == 0 & Babies == 0 ~ 'Couple',
                                                           Adults == 1 & (Children > 0 | Babies > 0) ~ 'Single Parent',
                                                           Adults == 2 & (Children > 0 | Babies > 0) ~ 'Family',
                                                           Adults > 2 ~ 'Group')) %>% mutate(HouseholdType = as.factor(HouseholdType))
```
...
```{r}
df_bookings <- df_bookings %>% subset(AverageDailyRate > 0) %>% drop_na(HouseholdType)
#df_bookings <- df_bookings %>% 
...
ggplot(df_bookings, aes(HouseholdType, TotalSpecialRequests)) + 
  ...
  facet_wrap(~BookingCancelled, ncol = 2, labeller = labeller(BookingCancelled = 
    c("No" = "Not cancelled bookings",
...
ggplot(df_bookings, aes(TotalNights, Price, color = AssignedRoomType)) +
  stat_summary(fun = "mean", geom = "point")
ggplot(df_bookings, aes(TotalNights, Price)) +
...
```{r}
df_bookings %>% filter(ArrivalDateYear == 2016, AssignedRoomType == "A") %>% 
...
```{r}
df_bookings %>%  subset(AssignedRoomType == "A" | AssignedRoomType =="C"| AssignedRoomType =="D"| AssignedRoomType =="E"| AssignedRoomType =="F"| AssignedRoomType =="G") %>%
  ggplot(aes(AverageDailyRate)) +
...
```{r}
sub <- paste("Correlation: ",round(cor(df_bookings$Price, df_bookings$TotalNights),3))
...
#Count outliers
count_outliers <- df_bookings %>% filter(Price >= upper_range_price | TotalNights >= upper_range_total_nights) %>% nrow()

df_bookings %>% ggplot(aes(x = Price, y = TotalNights)) +
  geom_jitter(alpha = 0.2, shape = 1) +
  xlim(0, upper_range_price) +
  ylim(0, upper_range_total_nights) +
  labs(title = "Correlation between Price and Total Nights", subtitle = paste('Removed ', count_outliers, 'outliers'), x = 'Price [EURO]', y = 'Total Nights')
```
...
```{r}
df_bookings %>% filter(BookingCancelled == 'No') %>% ggplot(aes(x=ArrivalDate)) +
  geom_density() +
  scale_x_date(date_labels = "%Y-%b", breaks = seq(as.Date("2016-06-01"), as.Date("2016-06-01")+180, "months"), limits = c(as.Date("2016-06-01"), c(as.Date("2016-06-01")+180)))+ theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5)) +
...
temp2 <- df_bookings %>% 
  filter(BookingCancelled == "No") %>%
    filter(ArrivalDate == temp[1,]|ArrivalDate == temp[2,]|ArrivalDate == temp[3,]|ArrivalDate ==temp[4,]|ArrivalDate == temp[5,]) %>%
  group_by(ArrivalDate) %>%
  summarise(n = sum(GroupSize)) 

temp_f <- merge(temp_f,temp2)

temp_f %>% 
  rename(
    date_of_arrival = ArrivalDate,
    number_of_reservations = Freq,
    number_of_people = n
    ) %>%
  arrange(desc(number_of_reservations))
