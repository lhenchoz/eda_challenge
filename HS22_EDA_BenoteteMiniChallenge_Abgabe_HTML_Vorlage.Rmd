...
Analysis: In total there are 13 NAs. By filtering the column for NAs, we can see that adults, children and babies is set to zero. The reason for this is unkown, but it is probably missing data. We can also say, that over 2/3 of the guests are couples.
...
Analysis: First we saw, that there is an observation with a negative value, which is invalid because it is not possible that customers pay a negative price for the hotel. Also we saw some observation with an average daily rate of 0, which we think is possible because there may be some customers with a coupon.
