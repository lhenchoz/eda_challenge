...
```{r}
df_bookings %>% filter(ArrivalDate >= "2016-01-01" & ArrivalDate <= "2016-12-31") %>% group_by(week = week(ArrivalDate)) %>% summarise(mean_week = mean(AverageDailyRate)) %>% ggplot(aes(x = week, y = mean_week)) +
  geom_point()
df_bookings %>% filter(ArrivalDate >= "2016-01-01" & ArrivalDate <= "2016-12-31") %>% group_by(week = week(ArrivalDate)) %>% summarise(mean_week = mean(AverageDailyRate)) %>% IQR(mean_week)
```
