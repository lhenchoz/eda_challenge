...
```{r}
df_bookings %>% filter(!complete.cases(.)) %>% count()
```
...
```{r}
df_bookings <- df_bookings %>% mutate(ArrivalDate = ymd(str_c(ArrivalDateYear, ArrivalDateMonth, ArrivalDateDayOfMonth, sep = "-")), .keep = 'unused')
```
...
```{r}
df_bookings <- df_bookings %>% mutate(across(c('BookingCancelled', 'Meal', 'MarketSegment', 'DistributionChannel', 'IsRepeatedGuest', 'ReservedRoomType', 'AssignedRoomType', 'DepositType', 'CustomerType', 'ReservationStatus'), as.factor))
