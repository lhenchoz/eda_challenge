...
```{r}
#TODO In einer Tabelle anzeigen
...
```{r}
kable(proportions(summary(df_bookings$HouseholdType)), caption = "HouseholdType", digits = 2, col.names = 'Frequency')
df_bookings %>% filter(is.na(HouseholdType))
```
Analysis:
In total there are 13 NAs. By filtering the column for NAs, we can see that adults, children and babies is set to zero. The reason for this is unkown. We can also say, that over 2/3 of the guests are couples.
...
```
Analysis:
There is a negative value as well as zeros which means that for some reason guests dont have to pay for the hotel.
...
```{r}
#TODO Was mit NAs in HouseholdType machen? Oder evtl andere Auffälligkeiten? Evtl. Median für Adults, Children setzten und neu erstellen lassen oder Werte Prozentual setzen? Stichwort Imputieren.
df_bookings <- df_bookings %>% 
  mutate(AverageDailyRate = replace(AverageDailyRate, AverageDailyRate <= 0, median(AverageDailyRate, na.rm = TRUE)))
```
